...
public class TestJoinOperator {
  ...
  public void testSimpleJoin() throws QueryPlanException, DatabaseException, IOException {
    ...
    public void testHardPNLJ() throws QueryPlanException, DatabaseException, IOException {
        File tempDir = tempFolder.newFolder("joinTest");
        ...
        for (int i = 0; i < 1000; i++) {
            List<DataBox> tempLeft = TestUtils.createRecordWithAllTypesWithValue(i).getValues();
            List<DataBox> tempRight = TestUtils.createRecordWithAllTypesWithValue(999 - i).getValues();
            transaction.addRecord("leftTable", tempLeft);
            transaction.addRecord("leftTable", tempLeft);
            transaction.addRecord("rightTable", tempRight);
            transaction.addRecord("rightTable", tempRight);
        }
        ...
        QueryOperator qo = new PNLJOperator(s1, s2, "int", "int", transaction);
        ...
        assertEquals(1000*4, count*4);
    }

    @Test
    @Category(StudentTestP3.class)
    public void testHardBNLJ() throws QueryPlanException, DatabaseException, IOException {
        File tempDir = tempFolder.newFolder("joinTest");
        Database d = new Database(tempDir.getAbsolutePath(), 4);
        Database.Transaction transaction = d.beginTransaction();
        ...
        QueryOperator qo = new BNLJOperator(s1, s2, "int", "int", transaction);
        Iterator<Record> iter = qo.iterator();
        int count = 0;
    ...
    @Test
    @Category(StudentTestP3.class)
    public void testHardSortMergeJoin() throws QueryPlanException, DatabaseException, IOException {
        File tempDir = tempFolder.newFolder("joinTest");
        Database d = new Database(tempDir.getAbsolutePath(), 4);
        Database.Transaction transaction = d.beginTransaction();
        ...
        QueryOperator qo = new SortMergeOperator(s1, s2, "int", "int", transaction);
        Iterator<Record> iter = qo.iterator();
        int count = 0;

        while (iter.hasNext()) {
            List<DataBox> temp1 = TestUtils.createRecordWithAllTypesWithValue(count).getValues();
            List<DataBox> temp2 = TestUtils.createRecordWithAllTypesWithValue(count).getValues();
            temp1.addAll(temp2);
            Record expected = new Record(temp1);
