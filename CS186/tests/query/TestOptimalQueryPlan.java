...
public class TestOptimalQueryPlan {
  ...
  @Test
  @Category(StudentTestP4.class)
  public void testComplexQuery() throws DatabaseException, QueryPlanException {
    Database.Transaction transaction = database.beginTransaction();
    QueryPlan queryPlan = transaction.query(defaulTableName);
    ...
    while (outputIterator.hasNext()) {
      ...
      assertTrue(record.getValues().get(0).getInt() == 1);
      assertTrue(record.getValues().get(1).getInt() == 1);
      assertTrue(record.getValues().get(2).getInt() == 1);
      assertTrue(record.getValues().get(3).getFloat() == 1.0f);
    }
