...
public class TestIntHistogram {
  ...
  public void testEmptyHistogram() {
    ...
    IntDataBox value = new IntDataBox(999);
  ...
  @Test
  @Category(StudentTestP4.class)
  public void testcomputeReductionFactorHard() {
    IntHistogram histogram = new IntHistogram();
    ...
    assertEquals(960, histogram.getNumDistinct());

    IntDataBox equalsValue = new IntDataBox(42);
    assertEquals(0.001041666666667,
            histogram.computeReductionFactor(PredicateOperator.EQUALS,
                                            equalsValue),
            0.000001f);

    IntDataBox lessThanValue = new IntDataBox(186);
    assertEquals(0.19375,
            histogram.computeReductionFactor(PredicateOperator.LESS_THAN,
                                            lessThanValue),
            0.000001f);

    IntDataBox lessThanEqualsValue = new IntDataBox(186);
    assertEquals(0.194791666666667,
            histogram.computeReductionFactor(PredicateOperator.LESS_THAN_EQUALS,
                                            lessThanEqualsValue),
            0.000001f);

    IntDataBox greaterThanValue = new IntDataBox(500);
    assertEquals(0.479166666666667,
            histogram.computeReductionFactor(PredicateOperator.GREATER_THAN,
                                            greaterThanValue),
            0.000001f);

    IntDataBox greaterThanEqualsValue = new IntDataBox(500);
    assertEquals(0.480208333333333,
            histogram.computeReductionFactor(PredicateOperator.GREATER_THAN_EQUALS,
