...
class ExactInference(InferenceModule):
	...
	def elapseTime(self, gameState):
		...
		"""
		"*** YOUR CODE HERE ***"
		beliefsNext = DiscreteDistribution()

		for position in self.allPositions:
			newPosDist = self.getPositionDistribution(gameState, position)

			for newPos in newPosDist:
				beliefsNext[newPos] += newPosDist[newPos] * self.beliefs[position]

		pacmanPosition = gameState.getPacmanPosition()
		beliefsNext[pacmanPosition] = 0

		self.beliefs = beliefsNext
...
class ParticleFilter(InferenceModule):
	...
	def observeUpdate(self, observation, gameState):
		...
		for position in beliefs:
			beliefs[position] *= self.getObservationProb(self.obs, pacmanPosition, position, jailPosition)
	...
	def elapseTime(self, gameState):
		...
		for particle in self.particles:
			...
			particlesNext += [DiscreteDistribution(newPosDist).sample()]

		self.particles = particlesNext
...
class JointParticleFilter(ParticleFilter):
	...
	def observeUpdate(self, observation, gameState):
		...
		for i in range(self.numGhosts):
			...
			for position in beliefs:
				beliefs[position] *= self.getObservationProb(observation[i], pacmanPosition, position[i], jailPosition)
	...
	def elapseTime(self, gameState):
		...
		for oldParticle in self.particles:
			...
			for i in range(self.numGhosts):
				...
				newParticle[i] = DiscreteDistribution(newPosDist).sample()
