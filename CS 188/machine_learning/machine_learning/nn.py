...
class Graph(object):
	...
	def get_inputs(self, node):
		...
		"""
		"*** YOUR CODE HERE ***"
		return self.inputs[self.nodes.index(node)]
	...
	def add(self, node):
		...
		self.nodes += [node]
		self.inputs += [inputs]
		self.outputs += [node.forward(self.get_inputs(node))]
		self.gradients += [np.zeros_like(self.get_output(node))]
	...
	def backprop(self):
		...
		"*** YOUR CODE HERE ***"
		self.gradients[self.nodes.index(loss_node)] = np.array(1.0)

		parents = loss_node.get_parents()
		parent_index = 0

		for parent in parents:
			self.backprop_helper(loss_node, parent, parent_index, np.array(1.0))
			parent_index += 1

	def backprop_helper(self, child, node, index, gradient):
		curr_gradient = child.backward(self.get_inputs(child), gradient)[index]
		self.gradients[self.nodes.index(node)] += curr_gradient

		parents = node.get_parents()
		parent_index = 0

		for parent in parents:
			self.backprop_helper(node, parent, parent_index, self.gradients[self.nodes.index(node)])
			parent_index += 1
...
class ReLU(FunctionNode):
	...
	def forward(inputs):
		"*** YOUR CODE HERE ***"
		return np.where(inputs[0] < 0, np.zeros(np.shape(inputs[0])), inputs[0])

	@staticmethod
	def backward(inputs, gradient):
		"*** YOUR CODE HERE ***"
		return [np.where(inputs[0] < 0, np.zeros(np.shape(inputs[0])), gradient)]
...
class SquareLoss(FunctionNode):
	...
	def backward(inputs, gradient):
		...
		return [c * (inputs[0] - inputs[1]), c * (inputs[1] - inputs[0])]
