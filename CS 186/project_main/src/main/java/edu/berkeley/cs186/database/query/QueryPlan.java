...
public class QueryPlan {
   ...
   * @throws QueryPlanException
   */
  private QueryOperator pushDownSelects(QueryOperator source, int except, String table) throws QueryPlanException, DatabaseException {
    /* TODO: Implement me! */
    int index = 0;
    ...
    for (String selectColumn : selectColumnNames) {
      ...
      int splitIndex = selectColumn.indexOf(".");

      if (splitIndex == -1) {
        source = new SelectOperator(source, selectColumn, selectOperators.get(index), selectDataBoxes.get(index));
        index++;
        continue;
      }

      if (!selectColumn.substring(0, splitIndex).equals(table)) {
        index++;
        continue;
      }

      selectColumn = selectColumn.substring(splitIndex + 1);
      source = new SelectOperator(source, selectColumn, selectOperators.get(index), selectDataBoxes.get(index));
      index++;
    }
  ...
  private QueryOperator minCostSingleAccess(String table) throws DatabaseException, QueryPlanException {
    ...
    for (int i : selectIndices) {
      System.out.println("fdfadf");
      IndexScanOperator indexOp = new IndexScanOperator(transaction, table, selectColumnNames.get(i),
              selectOperators.get(i), selectDataBoxes.get(i));
      int indexCost = indexOp.getIOCost();
    ...
    minOp = this.pushDownSelects(minOp, minSelectIdx, table);
    return minOp;
  }
  ...
  private QueryOperator minCostJoinType(QueryOperator leftOp,
    ...
    PNLJOperator PNLJ = new PNLJOperator(leftOp, rightOp, leftColumn, rightColumn, transaction);
    ...
    BNLJOperator BNLJ = new BNLJOperator(leftOp, rightOp, leftColumn, rightColumn, transaction);
    ...
    GraceHashOperator GHOp = new GraceHashOperator(leftOp, rightOp, leftColumn, rightColumn, transaction);
    if (GHOp.getIOCost() < minCost) {
      minOp = GHOp;
    }
  ...
  private Map<Set, QueryOperator> minCostJoins(Map<Set, QueryOperator> prevMap,
    ...
    int opCost = -1;
    Set<String> key0 = null;
    Set<String> key1 = null;
    Set<String> key2 = null;

    for (int i = 0; i < joinTableNames.size(); i++) {
      String leftTable = joinLeftColumnNames.get(i);
      String rightTable = joinRightColumnNames.get(i);
      int leftSplitIndex = leftTable.indexOf(".");
      int rightSplitIndex = rightTable.indexOf(".");

      String leftTableName = leftTable.substring(0, leftSplitIndex);
      String rightTableName = rightTable.substring(0, rightSplitIndex);
      String leftColumnName = leftTable.substring(leftSplitIndex + 1);
      String rightColumnName = rightTable.substring(rightSplitIndex + 1);

      Set<String> leftKey = new HashSet<String>();
      leftKey.add(leftTableName);
      Set<String> rightKey = new HashSet<String>();
      rightKey.add(rightTableName);

      QueryOperator tempOp = minCostJoinType(pass1Map.get(leftKey), pass1Map.get(rightKey),
              leftColumnName, rightColumnName);
      int tempCost = tempOp.getIOCost();

      if (tempCost < opCost || opCost == -1) {
        key0 = new HashSet<String>();
        key1 = new HashSet<String>();
        key2 = new HashSet<String>();

        key0.add(leftTableName);
        key0.add(rightTableName);
        key1.add(leftTableName);
        key2.add(rightTableName);
    ...
    pass1Map.remove(key1);
    pass1Map.remove(key2);
