...
class StudentUSocket(StudentUSocketBase):
  ...
  def tx(self, p, retxed=False):
    ...
    if not retxed:
      ...
      if p.tcp.SYN or p.tcp.FIN or len(p.tcp.payload) > 0:
  ...
  def rx(self, p):
    ...
    * Send pending ACKs and FINs
    ...
    while not self.rx_queue.empty():
      ...
      seq_no, packet = self.rx_queue.pop()
  ...
  def update_rto(self, acked_pkt):
    ...
    if self.srtt == 0 and self.rttvar == 0:
      ...
      self.rttvar = (1.0 - self.beta) * self.rttvar + self.beta * abs(self.srtt - R)
  ...
  def maybe_send(self):
    ...
    bytes_sent = 0
    # Complete for Stage 4
    remaining = min(self.snd.wnd, len(self.tx_data))
    self.log.debug("AAAAAAA {0}".format(len(self.tx_data)))
    while remaining > 0:
      payload = self.tx_data[:min(self.mss, self.snd.wnd - bytes_sent, remaining)]
  ...
  def check_timer_retx(self):
    ...
    if time_in_queue >= self.rto:
      ...
      self.retx_queue.q[0][1].retxed = True
